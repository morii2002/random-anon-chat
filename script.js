...
const affImage = document.getElementById('affImage');
...
// OGP画像版: title + image + description + URLを表示
...
async function updateAffiliate() {
  try {
    ...
    if (data.link) {
      ...
      affImage.src = data.link.image || '';
      affTitle.textContent = data.link.title || data.link.text || 'Link';
      affDesc.textContent = data.link.description || '';
    } else {
